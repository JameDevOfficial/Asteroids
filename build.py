...
def create_windows_exe(love_file, love_version, arch, output_dir):
    ...
    game_exe = love_dir / "asteroids.exe"
    
    print(f"Creating asteroids.exe ({arch}-bit)...")
    ...
    # Create zip archive
    output_zip = output_dir / f"Asteroids-win{arch}.zip"
...
def main():
    ...
    web = releases / "web"
    love_file = releases / "Asteroids.love"
    ...
    checks = {
        ".love file": love_file,
        "Windows 32-bit": executables / "Asteroids-win32.zip",
        "Windows 64-bit": executables / "Asteroids-win64.zip", 
